...
def ask(prompt: str, default: str = "s") -> bool:
...
def main():
    ...
    if ask("Limpar cache do VS Code (Cache, CachedData, GPUCache)?"):
        clean_vscode_cache()
    else:
        print("  ⏭️  Cache do VS Code ignorado.")

    if ask("Limpar workspaceStorage do VS Code? (reseta configurações por workspace)"):
        clean_vscode_workspace_storage()
    else:
        print("  ⏭️  workspaceStorage ignorado.")
