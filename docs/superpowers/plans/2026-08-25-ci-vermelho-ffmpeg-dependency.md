...
- **Baseline a preservar:** localmente, sem ffmpeg no PATH, hoje: `2 failed, 423 passed` em `enhance/` (isolado) ou `3 failed, 432 passed` na suíte completa (`test_render_queue.py enhance/ ui/ tools/` — a terceira falha, em `tools/test_generate_hollywood_lut_cooler.py::test_structure`, é um artefato de CRLF/LF do ambiente local, **não existe no CI real** e está fora do escopo deste ciclo). Meta ao final: `425 passed` na suíte completa (contando os testes de `tools/`), com a falha de CRLF ausente por não ser causada por este ciclo.
...
- **Risco residual:** a falha de `tools/test_generate_hollywood_lut_cooler.py::test_structure` observada localmente (contagem de `\r\n`) não é deste ciclo — é artefato de ambiente (confirmado ausente no log real do CI) e já foi revertido na árvore de trabalho antes deste plano ser escrito. Se ela reaparecer no CI real (não apenas localmente), é achado novo, não uma regressão desta correção.
