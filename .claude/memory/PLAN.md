...
| AE6 | A/B real: encodar o mesmo source com a LUT antiga e a nova, e rodar QC de entrega no output novo. Confirmar que conformidade Instagram e VMAF-contra-o-source não regrediram — a mudança é só croma, VMAF não deve mover de forma material. Colar saída literal em `STATE.md`. | `encode-validator` | `.claude/memory/STATE.md` | **done** — par trocado para `Captions_C32BA2` (indicado pelo usuário, baseline de 21/ago vs 18/ago); `validate_encode.sh` **19 ✅ / 1 ⚠ / 0 ❌ idêntico nos dois lados**, 10/10 checks do `.qc.json` sem mudança de estado, VMAF não-NEG `90.20 → 90.23`. O ⚠ (`TP −1.4`) é pré-existente, já `passed:false` no QC de 21/ago. Incidente: `AEF1` em `FINDINGS.md` |
